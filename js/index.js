...
const STANDARD_COLOR = getComputedStyle(document.body).getPropertyValue('--cell-color');

// det color of blocks from CSS variable
const BLOCK_COLOR = getComputedStyle(document.body).getPropertyValue('--block-color');
...
for (let i = 0; i < 25; i++) {
    ...
    for (let j = 0; j < 10; j++) {
        ...
        let tempCell = document.getElementById(id);
...
function temporaryShowInner() {
    ...
    for (let i = 0; i < 25; i++) {
        for (let j = 0; j < 10; j++) {
            ...
            tempIndex = occupiedArray[i][j].toString();
            tempCell = document.getElementById(id);
            tempCell.innerHTML = tempIndex;
        }
    }
...
// Returns 1 if ok to render, 0 if rendering should be skipped, 
// and 2 if rendering is not possible AND the block is 'newborn' - in this case game over
function checkBeforeRendering(plannedBlock) {
    for (let i = 0; i < plannedBlock.idArray.length; i++) {
        if (occupiedArray[plannedBlock.idArray[i][0] - 1][plannedBlock.idArray[i][1] - 1] === 2) {
            occupiedArray[plannedBlock.idArray[i][0] - 1][plannedBlock.idArray[i][1] - 1] = plannedBlock.colors[i];
        }
    }
...
function render(block) {
    ...
    for (let i = 0; i < block.idArray.length; i++) {
        id = block.idArray[i][0].toString() + "-" + block.idArray[i][1].toString();
            cell = document.getElementById(id);
        if (block.colors[i]) {
            cell.style.backgroundColor = BLOCK_COLOR;
        } else if (occupiedArray[block.idArray[i][0] - 1][block.idArray[i][1] - 1] !== 2) {
...
function markOccupiedOrFree(block) {
    for (let i = 0; i < block.idArray.length; i++) {
        if (occupiedArray[block.idArray[i][0] - 1][block.idArray[i][1] - 1] !== 2) {
            occupiedArray[block.idArray[i][0] - 1][block.idArray[i][1] - 1] = block.colors[i];
        }
    }
    // this section is temporary for development purposes. Delete later
    temporaryShowInner();
...
function rotateright() {
    ...
    let checkResult = checkBeforeRendering(currentBlock);
    if (!checkResult) {
...
function prepareToShiftHorizontal(direction) {
    ...
    for (let i = 0; i < currentBlock.idArray.length; i++) {
        ...
        if (occupiedArray[currentBlock.idArray[i][0] - 1 + direction][currentBlock.idArray[i][1] - 1] === 2
    ...
    if (direction < 0 && copyBlock.idArray[2][1] < 10) {
        beingLeftBehind = [copyBlock.idArray[2], copyBlock.idArray[5], copyBlock.idArray[8]];
    } else if (direction > 0 && copyBlock.idArray[0][1] > 1) {
        beingLeftBehind = [copyBlock.idArray[0], copyBlock.idArray[3], copyBlock.idArray[6]];
    }
    for (let j = 0; j < 3; j++) {
        if (occupiedArray[beingLeftBehind[0] - 1, beingLeftBehind[1] - 1] !== 2) {
            occupiedArray[beingLeftBehind[0] - 1, beingLeftBehind[1] - 1] = 0;
            let id = beingLeftBehind[0].toString() + "-" + beingLeftBehind[1].toString();
